...
let drag = { active:false, startX:0, startY:0, camX0:0, camY0:0 };
let paused     = false;
...
canvas.addEventListener('mousedown', e => {
  ...
  drag.camX0  = cam.x;     drag.camY0  = cam.y;
...
// ══════════════════════════════════════════════
// RESIZE — uses ResizeObserver so it works on
// GitHub Pages, slow loads, and all browsers
// ══════════════════════════════════════════════
function resizeCanvas() {
  // getBoundingClientRect is more reliable than clientWidth
  // when the element is inside a flex/grid container
  const rect = canvas.getBoundingClientRect();
...
// Watch the canvas for size changes (handles resize, sidebar drag, etc.)
...
// Also fire on load + a few delayed retries in case layout is still settling
resizeCanvas();
...
function initSim(p) {
  ...
  return {
    ...p,
    x:0, y:p.h0,
    vx:p.v0*Math.cos(rad), vy:p.v0*Math.sin(rad),
    t:0,
    trail:[], vyArr:[], vxArr:[], hdArr:[], spArr:[],
    fDx:0, fDy:0,
...
function tgtPos(futureT) {
  if (tgt.mode === 'intercept') return { x:tgt.baseX, y:tgt.baseY };
  // Evasive: linear prediction from current position+velocity
  const dt = futureT - globalT;
  return { x:tgt.x + tgt.vx*dt, y:tgt.y + tgt.vy*dt };
}
...
function stepEnemy() {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (spd > 58) { tgt.vx *= 58/spd; tgt.vy *= 58/spd; }
    tgt.x += tgt.vx * DT;
    tgt.y += tgt.vy * DT;
    // Trail
    if (Math.random() < 0.25) tgt.trail.push({ x:tgt.x, y:tgt.y });
...
function stepSim(s, isPreview=false, tOffset=0) {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (v > 0) {
      ...
      s.fDx = -Fd*(s.vx/v); s.fDy = -Fd*(s.vy/v);
      ax = (s.fDx/MASS) + (s.wind/MASS);
      ay = (s.fDy/MASS) - s.g;
    } else {
      s.fDx=0; s.fDy=0; ax=s.wind/MASS; ay=-s.g;
    }
    ...
    if (!isPreview) {
      ...
      if (s.t%0.12 < DT+0.005) {
        s.vyArr.push({t:s.t,v:s.vy}); s.vxArr.push({t:s.t,v:s.vx});
        s.hdArr.push({x:s.x,y:s.y});  s.spArr.push({t:s.t,v:spd});
      }
      // Hit detection
    ...
    if (s.y<=0) {
      if (s.bounce>0.01 && Math.abs(s.vy)>1.5) {
        ...
        s.y=0; s.done=true;
        if (!isPreview) {
          s.trail.push({x:s.x,y:0,speed:0});
...
function spawnExplosion(x,y,scale=1) {
  explosions.push({x,y,life:1.0,r:0,maxR:48*scale,scale});
  const fc=Math.round(40*scale);
  for(let i=0;i<fc;i++){const a=Math.random()*Math.PI*2,sp=12+Math.random()*60*scale;spawnParticle(x,y,Math.cos(a)*sp,Math.sin(a)*sp,'fire');}
  const sc=Math.round(16*scale);
  for(let i=0;i<sc;i++){const a=Math.random()*Math.PI*2,sp=6+Math.random()*20;spawnParticle(x,y,Math.cos(a)*sp,Math.sin(a)*sp,'smoke');}
  const dc=Math.round(14*scale);
  for(let i=0;i<dc;i++){
    const a=Math.random()*Math.PI*2,sp=20+Math.random()*65;
    particles.push({x,y,vx:Math.cos(a)*sp+(Math.random()-.5)*8,vy:Math.sin(a)*sp+(Math.random()-.5)*8,life:1.0,decay:0.01+Math.random()*0.015,type:'debris',r:1.5+Math.random()*2.5});
  }
}
function updateExplosions() {
  explosions.forEach(e=>{e.r+=(e.maxR-e.r)*.18;e.life-=0.026;});
...
function updateCamera() {
  ...
  const W=canvas.width,H=canvas.height;
  let minX=-25,maxX=70,minY=-10,maxY=55;
  const h0=+document.getElementById('sl-h0').value;
  if(h0>maxY) maxY=h0+12;
  if(tgt.active){minX=Math.min(minX,tgt.x-28);maxX=Math.max(maxX,tgt.x+28);maxY=Math.max(maxY,tgt.y+28);}
  sims.filter(s=>!s.done||s.t<10).forEach(s=>{
    minX=Math.min(minX,s.x-18);maxX=Math.max(maxX,s.x+18);maxY=Math.max(maxY,s.y+18);
  });
  if(preview) preview.forEach(p=>{minX=Math.min(minX,p.x-6);maxX=Math.max(maxX,p.x+12);maxY=Math.max(maxY,p.y+12);});
  const pad=0.83;
  ...
  const l=0.07;
  cam.scale+=(cam.ts-cam.scale)*l; cam.x+=(cam.tx-cam.x)*l; cam.y+=(cam.ty-cam.y)*l;
}

function toggleAutoTrack() {
  // If follow mode is on, turn it off first
  if (followMode) { toggleFollow(); return; }
  manualCam = !manualCam;
  const badge = document.getElementById('cam-badge');
  if (manualCam) {
    badge.textContent = 'MANUAL — click to auto-track';
...
function togglePause() {
  ...
  if (paused) {
    btn.textContent = '▶ RESUME'; btn.classList.add('btn-active');
    if(mobBtn){mobBtn.textContent='▶ RESUME';mobBtn.classList.add('btn-active');}
    overlay.style.display = 'flex';
  } else {
    btn.textContent = '⏸ PAUSE'; btn.classList.remove('btn-active');
    if(mobBtn){mobBtn.textContent='⏸ PAUSE';mobBtn.classList.remove('btn-active');}
...
function wy(Y){return canvas.height-((Y-cam.y)*cam.scale);}
...
function drawRocket(sx,sy,vx,vy,isHit) {
  if(!document.getElementById('tog-rocket').checked) {
    ctx.beginPath();ctx.arc(sx,sy,Math.max(3,1.5*cam.scale),0,Math.PI*2);
    ctx.fillStyle=isHit?'#10c97e':'#4f8ef7';ctx.fill();
    ctx.strokeStyle='#fff';ctx.lineWidth=1;ctx.stroke();return;
  }
  const ang=Math.atan2(vy,vx), sz=Math.max(7,Math.min(20,cam.scale*2.8));
  ctx.save();ctx.translate(sx,sy);ctx.rotate(-ang+Math.PI/2);
  // Exhaust glow
  const g=ctx.createRadialGradient(0,sz*.6,0,0,sz*.6,sz*.9);
  g.addColorStop(0,'rgba(245,158,11,.78)');g.addColorStop(1,'rgba(245,158,11,0)');
  ctx.beginPath();ctx.arc(0,sz*.6,sz*.9,0,Math.PI*2);ctx.fillStyle=g;ctx.fill();
  // Body
  ctx.fillStyle='#e2e8f0';
  ctx.beginPath();ctx.roundRect(-sz*.17,-sz*.55,sz*.34,sz*1.05,sz*.05);ctx.fill();
  // Nose
  ctx.fillStyle=isHit?'#10c97e':'#4f8ef7';
  ctx.beginPath();ctx.moveTo(-sz*.17,-sz*.55);ctx.lineTo(0,-sz*.95);ctx.lineTo(sz*.17,-sz*.55);ctx.fill();
  // Window
  ctx.fillStyle=isHit?'rgba(16,201,126,.6)':'rgba(100,160,255,.5)';
  ctx.beginPath();ctx.arc(0,-sz*.26,sz*.1,0,Math.PI*2);ctx.fill();
  // Fins
  ctx.fillStyle='#94a3b8';
  ctx.beginPath();ctx.moveTo(-sz*.17,sz*.28);ctx.lineTo(-sz*.42,sz*.6);ctx.lineTo(-sz*.17,sz*.44);ctx.fill();
  ctx.beginPath();ctx.moveTo(sz*.17,sz*.28);ctx.lineTo(sz*.42,sz*.6);ctx.lineTo(sz*.17,sz*.44);ctx.fill();
  ctx.restore();
}
...
function drawEnemyRocket(sx,sy,vx,vy) {
  if(!document.getElementById('tog-rocket').checked) {
    ctx.beginPath();ctx.arc(sx,sy,Math.max(4,1.8*cam.scale),0,Math.PI*2);
    ctx.fillStyle='#f05050';ctx.fill();
    ctx.strokeStyle='#ff8080';ctx.lineWidth=1.5;ctx.stroke();return;
  }
  const ang=Math.atan2(vy,vx), sz=Math.max(7,Math.min(22,cam.scale*2.8));
  ctx.save();ctx.translate(sx,sy);ctx.rotate(-ang+Math.PI/2);
  // Blue thruster
  const g=ctx.createRadialGradient(0,sz*.6,0,0,sz*.6,sz*.9);
  g.addColorStop(0,'rgba(60,130,255,.85)');g.addColorStop(1,'rgba(60,130,255,0)');
  ctx.beginPath();ctx.arc(0,sz*.6,sz*.9,0,Math.PI*2);ctx.fillStyle=g;ctx.fill();
  // Body dark
  ctx.fillStyle='#280a0a';
  ctx.beginPath();ctx.roundRect(-sz*.17,-sz*.55,sz*.34,sz*1.05,sz*.05);ctx.fill();
  // Red stripe
  ctx.fillStyle='rgba(240,60,60,.4)';ctx.fillRect(-sz*.17,-sz*.12,sz*.34,sz*.2);
  // Red nose
  ctx.fillStyle='#ef4040';
  ctx.beginPath();ctx.moveTo(-sz*.17,-sz*.55);ctx.lineTo(0,-sz*.95);ctx.lineTo(sz*.17,-sz*.55);ctx.fill();
  // Warhead glow dot
  ctx.fillStyle='rgba(255,80,80,.75)';
  ctx.beginPath();ctx.arc(0,-sz*.84,sz*.09,0,Math.PI*2);ctx.fill();
  // Dark red fins
  ctx.fillStyle='#7a1a1a';
  ctx.beginPath();ctx.moveTo(-sz*.17,sz*.28);ctx.lineTo(-sz*.45,sz*.62);ctx.lineTo(-sz*.17,sz*.46);ctx.fill();
  ctx.beginPath();ctx.moveTo(sz*.17,sz*.28);ctx.lineTo(sz*.45,sz*.62);ctx.lineTo(sz*.17,sz*.46);ctx.fill();
  ctx.restore();
}
...
// ══════════════════════════════════════════════
function drawBase() {
  const bx=wx(0),by=wy(0);
  if (baseDestroyed) {
    ctx.fillStyle='#2a1208';ctx.beginPath();ctx.roundRect(bx-18,by-4,36,4,1);ctx.fill();
    ctx.fillStyle='#1c0c04';ctx.fillRect(bx-10,by-8,18,4);
    return;
  }
  // Bunker
  ctx.fillStyle='#151e30';ctx.strokeStyle='#253045';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.roundRect(bx-22,by-11,44,11,3);ctx.fill();ctx.stroke();
  ctx.fillStyle='#1c2840';ctx.fillRect(bx-18,by-15,36,4);
  // Mast
  ctx.strokeStyle='#3a4c6a';ctx.lineWidth=1;
  ctx.beginPath();ctx.moveTo(bx,by-15);ctx.lineTo(bx,by-27);ctx.stroke();
  // Dish
  ctx.strokeStyle='#4f8ef7';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.arc(bx,by-27,7,Math.PI+.4,Math.PI*2-.4);ctx.stroke();
  ctx.beginPath();ctx.arc(bx,by-27,4,Math.PI+.4,Math.PI*2-.4);ctx.stroke();
  ctx.fillStyle='#4f8ef7';ctx.beginPath();ctx.arc(bx,by-27,2.5,0,Math.PI*2);ctx.fill();
  // Label
  ctx.fillStyle='rgba(79,142,247,.65)';
  ctx.font=`${Math.max(7,Math.min(9,cam.scale))}px Share Tech Mono`;
  ctx.textAlign='center';ctx.fillText('BASE',bx,by-31);ctx.textAlign='left';
}
...
function drawTrail(trail,colorBySpeed,fixedColor,dashed=false) {
  ...
  if(colorBySpeed&&!fixedColor) {
    for(let i=1;i<trail.length;i++){
      ...
      ctx.beginPath();ctx.moveTo(wx(a.x),wy(a.y));ctx.lineTo(wx(b.x),wy(b.y));
      ctx.strokeStyle=speedColor((b.speed||0)/maxSpd);ctx.lineWidth=2.5;ctx.stroke();
    }
  } else {
    ctx.beginPath();ctx.moveTo(wx(trail[0].x),wy(trail[0].y));
    for(let i=1;i<trail.length;i++) ctx.lineTo(wx(trail[i].x),wy(trail[i].y));
    ctx.strokeStyle=fixedColor||'#4f8ef7';ctx.lineWidth=2.5;ctx.stroke();
  }
  ctx.setLineDash([]);
}
...
function drawArrow(sx,sy,vx,vy,col,scale=1) {
  ...
  if(len<3) return;
  const ang=Math.atan2(vy,vx),ex=sx+Math.cos(ang)*len,ey=sy-Math.sin(ang)*len;
...
function render() {
  ...
  if(document.getElementById('tog-grid').checked) {
    let step=10;
    if(cam.scale<1) step=100; else if(cam.scale<2.5) step=50; else if(cam.scale<6) step=25;
    ctx.strokeStyle='rgba(255,255,255,.03)';ctx.lineWidth=1;
    const startX=Math.floor(cam.x/step)*step;
    for(let x=startX;x<cam.x+W/cam.scale;x+=step){ctx.beginPath();ctx.moveTo(wx(x),0);ctx.lineTo(wx(x),H);ctx.stroke();}
    const startY=Math.floor(cam.y/step)*step;
    for(let y=startY;y<cam.y+H/cam.scale;y+=step){ctx.beginPath();ctx.moveTo(0,wy(y));ctx.lineTo(W,wy(y));ctx.stroke();}
    ctx.fillStyle='rgba(90,104,130,.6)';ctx.font=`${Math.max(8,Math.min(10,cam.scale*2))}px Share Tech Mono`;
    for(let x=startX;x<cam.x+W/cam.scale;x+=step) if(x>=0) ctx.fillText(x+'m',wx(x)+3,wy(0)+13);
    for(let y=startY+step;y<cam.y+H/cam.scale;y+=step) if(y>0) ctx.fillText(y+'m',wx(0)+3,wy(y)-3);
  }

  // Ground
  const gy=wy(0);
  ctx.strokeStyle='rgba(42,50,72,.95)';ctx.lineWidth=2;
  ctx.beginPath();ctx.moveTo(0,gy);ctx.lineTo(W,gy);ctx.stroke();
  const gfill=ctx.createLinearGradient(0,gy,0,gy+26);
  gfill.addColorStop(0,'rgba(22,32,50,.6)');gfill.addColorStop(1,'rgba(22,32,50,0)');
  ctx.fillStyle=gfill;ctx.fillRect(0,gy,W,26);

  // Launch tower
  const h0=+document.getElementById('sl-h0').value;
  if(h0>0) {
    const tw=4*cam.scale,th=h0*cam.scale,tx=wx(0)-tw/2,ty=wy(h0);
    ctx.fillStyle='#101820';ctx.fillRect(tx,ty,tw,th);
    ctx.strokeStyle='rgba(38,48,68,.9)';ctx.lineWidth=1;ctx.strokeRect(tx,ty,tw,th);
    const struts=Math.max(1,Math.floor(h0/18));
    ctx.strokeStyle='rgba(32,42,60,.8)';
    for(let i=1;i<struts;i++){const sy2=wy(i*(h0/struts));ctx.beginPath();ctx.moveTo(tx,sy2);ctx.lineTo(tx+tw,sy2);ctx.stroke();}
  }
  ...
  if(tgt.active && tgt.mode==='intercept' && !tgt.hit) {
    ...
    const br=tgt.r*cam.scale*1.7, ex2=wx(tgt.x), ey2=wy(tgt.y);
    ctx.strokeStyle='rgba(240,80,80,.55)';ctx.lineWidth=1.5;
    [[1,1],[1,-1],[-1,1],[-1,-1]].forEach(([sx2,sy2])=>{
      ctx.beginPath();ctx.moveTo(ex2+sx2*br,ey2+sy2*br);ctx.lineTo(ex2+sx2*br-sx2*9,ey2+sy2*br);ctx.stroke();
      ctx.beginPath();ctx.moveTo(ex2+sx2*br,ey2+sy2*br);ctx.lineTo(ex2+sx2*br,ey2+sy2*br-sy2*9);ctx.stroke();
    });
    ctx.beginPath();ctx.arc(ex2,ey2,tgt.r*cam.scale*2.4,0,Math.PI*2);
    ctx.strokeStyle='rgba(240,80,80,.1)';ctx.lineWidth=1;ctx.stroke();
  ...
  if(tgt.active && tgt.mode==='evasive' && !tgt.hit && !tgt.reachedBase) {
    ...
    if(tgt.trail.length>1) {
      ctx.beginPath();ctx.moveTo(wx(tgt.trail[0].x),wy(tgt.trail[0].y));
      for(let i=1;i<tgt.trail.length;i++) ctx.lineTo(wx(tgt.trail[i].x),wy(tgt.trail[i].y));
      ctx.strokeStyle='rgba(220,50,50,.3)';ctx.lineWidth=1.8;ctx.stroke();
    }
    ...
    if(proximity>0) {
      ctx.fillStyle=`rgba(220,40,40,${proximity*.14})`;ctx.fillRect(0,0,W,H);
    }
  }

  // Ghost preview
  if(preview && document.getElementById('tog-preview').checked) {
    ctx.globalAlpha=.28;drawTrail(preview,false,'#64748b',true);ctx.globalAlpha=1;
  }
  ...
  explosions.forEach(e=>{
    const ex=wx(e.x),ey=wy(e.y),fr=e.r*cam.scale;
    // Fireball gradient
    ...
    ctx.beginPath();ctx.arc(ex,ey,fr,0,Math.PI*2);ctx.fillStyle=fg;ctx.fill();
    // Primary shockwave
    const ringR=e.maxR*(1-e.life)*cam.scale*1.85+fr*.3;
    ctx.beginPath();ctx.arc(ex,ey,ringR,0,Math.PI*2);
    ctx.strokeStyle=`rgba(255,200,80,${e.life*.38})`;ctx.lineWidth=2.5;ctx.stroke();
    // Secondary ring
    ctx.beginPath();ctx.arc(ex,ey,ringR*1.45,0,Math.PI*2);
    ctx.strokeStyle=`rgba(180,80,30,${e.life*.18})`;ctx.lineWidth=1;ctx.stroke();
  });
...
// graph config lookup
const GRAPH_CFG = {
  'g-vy': { label:'VY / TIME', xKey:'t', yKey:'v', color:'rgb(16,201,126)',  data:()=>sims.length?sims[sims.length-1].vyArr:[] },
  'g-vx': { label:'VX / TIME', xKey:'t', yKey:'v', color:'rgb(79,142,247)',  data:()=>sims.length?sims[sims.length-1].vxArr:[] },
  'g-hd': { label:'HEIGHT / DIST', xKey:'x', yKey:'y', color:'rgb(245,158,11)', data:()=>sims.length?sims[sims.length-1].hdArr:[] },
  'g-sp': { label:'SPEED / TIME', xKey:'t', yKey:'v', color:'rgb(167,139,250)',data:()=>sims.length?sims[sims.length-1].spArr:[] },
};
...
function drawGraph(idOrCanvas, data, xKey, yKey, color) {
  ...
  // Zero line
  if(minY<0&&maxY>0){
    gctx.strokeStyle='rgba(255,255,255,.08)';gctx.lineWidth=1;gctx.setLineDash([4,4]);
    gctx.beginPath();gctx.moveTo(pad,my(0));gctx.lineTo(W-pad,my(0));gctx.stroke();gctx.setLineDash([]);
  }
  ...
  if (typeof idOrCanvas !== 'string') {
    ...
    for(let i=0;i<=steps;i++){
      ...
      gctx.beginPath();gctx.moveTo(pad,my(v));gctx.lineTo(W-pad,my(v));gctx.stroke();
    }
  }
  ...
  gctx.beginPath();gctx.moveTo(mx(data[0][xKey]),H-pad);
  ...
  gctx.strokeStyle=color;gctx.lineWidth=typeof idOrCanvas==='string'?1.5:2;gctx.stroke();

  // Axis labels
  const fs = typeof idOrCanvas==='string'?9:11;
  gctx.fillStyle='rgba(90,104,130,.85)'; gctx.font=`${fs}px Share Tech Mono`;
  if (typeof idOrCanvas === 'string') {
    ...
    gctx.beginPath();gctx.arc(mx(last[xKey]),my(last[yKey]),4,0,Math.PI*2);gctx.fill();
  }
}
...
function updateGraphs(){
  ...
  if (expandedGraph) {
    ...
    const mc  = document.getElementById('graph-modal-canvas');
    if (mc.width < 100) { mc.width=mc.clientWidth||760; mc.height=mc.clientHeight||320; }
    drawGraph(mc, cfg.data(), cfg.xKey, cfg.yKey, cfg.color);
  }
}
...
function restartMission(){
  ...
  baseDestroyed=false;killCount=0;
  document.getElementById('st-kills').textContent='0';
  sims=[];particles=[];explosions=[];preview=null;globalT=0;screenFlash=0;
...
function runAutoAim(){
  ...
  setTimeout(()=>{
    const env=getParams();
    let bd=Infinity,ba=env.theta,bv=env.v0;
    for(let v=10;v<=280;v+=12){
      for(let a=5;a<=85;a+=2.5){
        const s=initSim({...env,v0:v,theta:a,bounce:0});
        let md=Infinity;
        while(!s.done&&s.t<30){stepSim(s,true,globalT);const pt=tgtPos(globalT+s.t);const d=Math.hypot(s.x-pt.x,s.y-pt.y);if(d<md)md=d;if(d<=tgt.r)break;}
        if(md<bd){bd=md;ba=a;bv=v;}if(bd<=tgt.r)break;
      }if(bd<=tgt.r)break;
    }
    for(let v=Math.max(5,bv-12);v<=bv+12;v+=1){
      for(let a=Math.max(1,ba-4);a<=Math.min(89,ba+4);a+=.5){
        ...
        let md=Infinity;
        while(!s.done&&s.t<30){stepSim(s,true,globalT);const pt=tgtPos(globalT+s.t);const d=Math.hypot(s.x-pt.x,s.y-pt.y);if(d<md)md=d;if(d<=tgt.r)break;}
        if(md<bd){bd=md;ba=a;bv=v;}
      }
    }
    document.getElementById('sl-ang').value=ba;document.getElementById('v-ang').textContent=ba.toFixed(1)+'°';
    document.getElementById('sl-v0').value=bv;document.getElementById('v-v0').textContent=bv+' m/s';
    buildPreview();
...
function launchVolley(){
  if(baseDestroyed) return;
  const p=getParams();
  for(let i=-2;i<=2;i++) sims.push(initSim({...p,theta:p.theta+i*4,v0:p.v0*(1+i*.02)}));
  if(sims.length>65) sims.splice(0,Math.max(0,sims.length-65));
  buildPreview();
}
function reset(){
  sims=[];particles=[];explosions=[];preview=null;
  globalT=0;screenFlash=0;baseDestroyed=false;killCount=0;
  document.getElementById('st-kills').textContent='0';
  document.getElementById('base-screen').classList.remove('show');
  if(tgt.active){tgt.hit=false;tgt.reachedBase=false;randomizeTarget();}
  ['st-maxh','st-range','st-time'].forEach(id=>document.getElementById(id).textContent='—');
  ['g-vy','g-vx','g-hd','g-sp'].forEach(id=>{const c=document.getElementById(id);c.getContext('2d').clearRect(0,0,c.width,c.height);});
  // Restore auto-track, clear pause & follow
  manualCam=false; paused=false; followMode=false; followScale=12;
  document.getElementById('cam-badge').textContent='AUTO-TRACK';
  document.getElementById('cam-badge').classList.remove('manual');
...
function randomizeTarget(){
  tgt.baseX=115+Math.random()*195; tgt.baseY=22+Math.random()*60;
  tgt.x=tgt.baseX; tgt.y=tgt.baseY;
  tgt.hit=false; tgt.reachedBase=false; tgt.trail=[];
  if(tgt.mode==='evasive'){
    const dx=-tgt.baseX,dy=-tgt.baseY,dist=Math.hypot(dx,dy);
    const speed=28+Math.random()*12;
    tgt.vx=(dx/dist)*speed; tgt.vy=(dy/dist)*speed;
  }
}
...
function toggleSlow(){slowMode=!slowMode;document.getElementById('btn-slow').classList.toggle('btn-active',slowMode);}
...
// SLIDER SYNC
// ══════════════════════════════════════════════
[['sl-h0','v-h0',v=>v+' m'],['sl-ang','v-ang',v=>v+'°'],['sl-v0','v-v0',v=>v+' m/s'],
 ['sl-bounce','v-bounce',v=>parseFloat(v).toFixed(2)],['sl-wind','v-wind',v=>v+' m/s'],
 ['sl-cd','v-cd',v=>parseFloat(v).toFixed(2)],['sl-rho','v-rho',v=>parseFloat(v).toFixed(3)],
 ['sl-g','v-g',v=>parseFloat(v).toFixed(2)+' m/s²'],
].forEach(([sid,lid,fmt])=>document.getElementById(sid).addEventListener('input',e=>{document.getElementById(lid).textContent=fmt(e.target.value);buildPreview();}));
...
document.getElementById('mission').addEventListener('change',e=>{
  ...
  tgt.mode=m;tgt.active=(m!=='free');tgt.hit=false;tgt.reachedBase=false;tgt.trail=[];
...
const PRESETS={earth:{g:9.81,rho:1.225},moon:{g:1.62,rho:0},mars:{g:3.72,rho:.02},jupiter:{g:24.8,rho:.16},vacuum:{g:9.81,rho:0,cd:0,wind:0}};
...
document.addEventListener('keydown',e=>{
  ...
  if(e.code==='KeyF'){e.preventDefault();toggleFollow();}
});
...
(function initResize(){
  ...
  let desktopDragging = false, startX = 0, startW = 0;
  ...
  let mobileDragging = false, startY = 0, startH = 0;
  ...
  window.addEventListener('mousemove', e => {
    ...
    const newH  = Math.min(maxH, Math.max(100, startH + delta));
...
(function run(){
  if(!paused){
    if(!baseDestroyed){
      ...
      if(tgt.active&&tgt.mode==='intercept'){tgt.x=tgt.baseX;tgt.y=tgt.baseY;}
      stepEnemy();
